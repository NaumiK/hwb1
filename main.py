...
class App:
    ...
    plot_functions: dict[
        int : dict[str : Callable[[pd.DataFrame, tuple[str]], go.Figure | None]]
    ] = {
        # Default
        0: {
            "Corr Heatmap": lambda df, xy: App.__check_category(
                df, xy, lambda a, b: px.imshow(a[list(b)].corr())
            ),
        },
        1: {
            ...
            "Density histogram": lambda df, xy: px.histogram(
                df, x=xy[0], histnorm="probability density"
            ),
        },
        2: {
        ...
        3: {
            "Scatter plot + color": lambda df, xy: px.scatter(
                df, x=xy[0], y=xy[1], color=xy[2]
            ),
        },
    }
    ...
    def ttest(col1: pd.Series, col2: pd.Series) -> None:
        st.subheader("T-Test")
        st.write(
            r"Let $X_1, \dots, X_{n_1} \sim N(\mu_1, \sigma_1)$, $Y_1, \dots, Y_{n_2} \sim N(\mu_2, \sigma_2)$"
        )
        ...
        alternative_choice = alternatives[
            st.selectbox(r"$H_1 \colon$", alternatives.keys())
        ]
        st.write(r"$\alpha: 0.05$")
        confirm = st.button("Confirm", key="T-Test confirm")
        if confirm:
            try:
                _, p_value = sp.stats.ttest_ind(
                    col1.dropna(),
                    col2.dropna(),
                    equal_var=False,
                ...
                if p_value < 0.05:
                    st.write(
                        r"Поскольку $\text{p-value} < 0,05$, мы отвергаем нулевую гипотезу ($H_0 \colon \mathbb{E}X=\mathbb{E}Y$)."
                    )
                else:
                    st.write(
                        r"Поскольку $\text{p-value} > 0,05$, мы не отвергаем нулевую гипотезу ($H_0 \colon \mathbb{E}X=\mathbb{E}Y$)."
                    )
    ...
    def utest(col1: pd.Series, col2: pd.Series) -> None:
        ...
        alternatives = {r"A ≠ B": "two-sided", r"A > B": "greater", r"A < B": "less"}
        ...
        if st.button("Confirm", key="T-Test confirm"):
            try:
                _, p_value = sp.stats.mannwhitneyu(
                    col1.dropna(), col2.dropna(), alternative=alternative_choice
                )
                st.write(f"p-value: {p_value}")
                if p_value < 0.05:
                    st.write(
                        r"Поскольку $\text{p-value} > 0,05$, мы отвергаем нулевую гипотезу ($A = B$)."
                    )
                else:
                    st.write(
                        r"Поскольку $\text{p-value} < 0,05$, мы не отвергаем нулевую гипотезу ($A = B$)."
                    )
    ...
    def show_info(self) -> None:
        ...
        info["Cat/Num"] = info[0].map(
            lambda x: "Number" if x in App.__numerics else "Category"
        )
    ...
    def choose_columns(self, dimension: int) -> tuple[str]:
        ...
        return tuple(
            st.selectbox(
                ...
                key=str(i + dimension * App.col_key),
            )
        ...
        if cols is not None:
            type_of_plot = st.selectbox(
                ...
                (
                    ...
                    if dimension in App.plot_functions
                    else []
                )
            ...
            if draw_plot:
                if (
                    dimension in App.plot_functions
                    and type_of_plot in App.plot_functions[dimension]
                ):
                    fig = App.plot_functions[dimension][type_of_plot](
                        accepted_query_df, cols
                    )
                else:
    ...
    def a_b_test(self) -> None:
        ...
        if col1 is not None and col2 is not None:
            App.test_functions[type_of_test](
                self.query_column(*col1), self.query_column(*col2)
            )
